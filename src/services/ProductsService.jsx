...
export const removeAllProducts = async (notify) => {
    ...
    const productsResponse = await fetch(API_URL + "/catalog/products", { headers: headers });
    if (!productsResponse.ok) throw new Error(`Erreur ${productsResponse.status}`);
    const products = await productsResponse.json();

    for (let i = 0; i < products.data.length; i++) {
        const product = products.data[i];
        const delResponse = await fetch(API_URL + `/catalog/products/${product.id}`, { headers: headers, method: "DELETE" });
